#ifndef __HEADER_INSTRUMENTS__
#define __HEADER_INSTRUMENTS__
...
class Inst : public RIFF {
public:
    Inst(wchar_t *path) { Load(path); }
    ~Inst() { Clear(); }
...
class ART_ : public RIFF {
public:
    ART_(FILE *fp, uint32 size);
    ~ART_();

protected:
    const char mId[4] = { 'a', 'r', 't', ' ' };
    const char* ID() override { return mId; }
};
